...
def generate_sku_master(n_skus: int = 500) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        "unit_cost":
            rng.uniform(10, 500, n_skus),

        "demand":
            rng.integers(1, 250, n_skus),

        "equipment_density_score":
            rng.uniform(0, 1, n_skus),

        "adi":
            rng.uniform(0.5, 2.0, n_skus),

        "cv_squared":
            rng.uniform(0.1, 2.5, n_skus),

        "depot_tier":
            rng.choice(depot_choices, n_skus),

        "environment_multiplier":
            rng.uniform(0.8, 1.4, n_skus),
        ...
        "lead_time_days":
            rng.integers(5, 180, n_skus).astype(float),

        "supply_origin_country":
            rng.choice(country_choices, n_skus),
        ...
        "hhi_score":
            rng.uniform(0.1, 1.0, n_skus),

        "mean_demand":
            rng.uniform(5, 200, n_skus),

        "std_demand":
            rng.uniform(1, 50, n_skus),

        "mean_lead_time":
            rng.uniform(5, 180, n_skus),

        "std_lead_time":
            rng.uniform(1, 30, n_skus),

        "stockout_cost_usd":
            rng.uniform(100, 50000, n_skus),
    })
