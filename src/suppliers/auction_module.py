...
class ProcurementAuctionEngine:
    ...
    def _generate_bidders_batch(
        ...
        margin_factor = 1.0 + ci_score * 0.25 + geo * 0.15

        cost_base = baseline_cost * rng.uniform(
            0.55,
            0.90,
            n,
        )

        vals = cost_base * margin_factor
        ...
        return {
            "id": np.array([f"SUP_{chr(65+i)}" for i in range(n)]),
            "geo": geo,
            "rep": rep,
            "vals": vals,
            "strat": strat,
    ...
    def _sim_sealed(
        ...
        baseline = float(row["unit_cost"])

        reserve = baseline * self.reserve_price_pct
        ...
        strat = np.array([rng.uniform(*strat_map[s]) for s in b["strat"]])

        rep_adj = 1.0 + (b["rep"] - 0.5) * 0.10

        geo_adj = 1.0 - b["geo"] * 0.15

        bids = b["vals"] * shading * rep_adj * geo_adj * strat
        ...
        valid_mask = (bids >= floor_price) & (bids <= reserve)
        ...
        local = np.argmin(bids[valid_idx])

        win_idx = valid_idx[local]

        win_bid = float(bids[win_idx])

        reduction = (baseline - win_bid) / baseline

        return AuctionResult(
            item_id=row["item_id"],
            auction_type="sealed_bid",
            winner_id=str(b["id"][win_idx]),
            winning_bid=round(
                win_bid,
                2,
            ),
    ...
    def _sim_english(
        ...
        while np.sum(active_mask) > 1 and round_num < self.max_rounds:

            round_num += 1

            threshold = current_price * (1 + self.english_increment)

            active_mask &= (b["vals"] >= threshold) & (threshold <= reserve)
        ...
        local = np.argmin(b["vals"][active_idx])
        ...
        reduction = (baseline - final_price) / baseline
    ...
    def _baseline_result(
        ...
        return AuctionResult(
            ...
            reserve_price=baseline * self.reserve_price_pct,
            n_rounds=1,
            n_bidders=n,
    ...
    def run_portfolio_auctions(
        ...
        eligible = sku_df[sku_df["ci_tier"].isin(target_tiers)].to_dict("records")
        ...
        def process(row_idx_row):
            ...
            use_english = (
                row.get("ci_score", 0.0) > 0.75 and row.get("ved_class") == "V"
            )
        ...
        results = Parallel(
            n_jobs=self.n_jobs,
            backend="loky",
        )(delayed(process)(x) for x in enumerate(eligible))
        ...
        return AuctionPortfolio(
            ...
            n_sealed_bid=int((rdf["type"] == "sealed_bid").sum()),
            n_english=int((rdf["type"] == "english").sum()),
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(
        {
            "item_id": [f"SKU{i:04d}" for i in range(500)],
