...
def compute_coalition_cost(
    ...
    for _, row in sku_df.iterrows():
        ...
        uc = float(row.get("unit_cost", 100.0))
        ...
        depot = str(row.get("depot_tier", "Rear"))

        if depot not in depot_subset and n < 3:
            continue

        base_hold = holding_rate * uc * q

        if n > 1:

            pooling_reduction = 1.0 - (1.0 - 1.0 / math.sqrt(n)) * pooling_factor

            hold_cost = base_hold * pooling_reduction
...
class ShapleyAllocator:
    ...
    def __init__(
        ...
        self.value_fn = value_function or compute_coalition_cost
    ...
    def _shapley_value_i(
        ...
        others = [d for d in self.depots if d != depot_i]
        ...
        for r in range(len(others) + 1):
            ...
            for combo in itertools.combinations(
                ...
                S_plus = frozenset(combo) | {depot_i}
    ...
    def allocate(self, sku_df):
        ...
        cv = self._all_coalition_values(sku_df)
        ...
        sum_solo = sum(solo_costs.values())
        ...
        total_skus = sum(depot_sku_counts.values())

        pro_rata_vals = {
            d: round(
                v_N * depot_sku_counts[d] / max(total_skus, 1),
                2,
            )
            for d in self.depots
        }
        ...
        for d in self.depots:
            ...
            sub = sku_df[sku_df["depot_tier"] == d]
            ...
            results.append(
                ShapleyResult(
                    ...
                    savings_vs_solo=round(
                        solo_costs[d] - shapley_vals[d],
                        2,
                    ),
                    savings_vs_pro_rata=round(
                        pro_rata_vals[d] - shapley_vals[d],
                        2,
                    ),
        ...
        for S, v_S in cv.items():
            ...
            coal_rows.append(
                {
                    "coalition": str(sorted(S)) if S else "∅",
                    "size": len(S),
                    "cost_v_S": v_S,
                }
            )

        coal_df = pd.DataFrame(coal_rows).sort_values(["size", "coalition"])

        efficiency_ok = abs(shapley_sum - v_N) < max(v_N * 0.01, 1.0)
    ...
    def log_to_mlflow(
        ...
        with mlflow.start_run(
            ...
            mlflow.log_metric(
                "efficiency_ok",
                int(alloc.efficiency_satisfied),
            )
    ...
    df = pd.DataFrame(
        {
            ...
            "depot_tier": ["Forward"] * 30 + ["Border"] * 35 + ["Rear"] * 25,
