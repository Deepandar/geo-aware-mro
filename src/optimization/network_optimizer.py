...
class MILPNetworkOptimizer:
    ...
    def solve(self, sku_df: pd.DataFrame):
        ...
        missing = [c for c in required if c not in sku_df.columns]
        ...
        geo_risk = sku_df.get("geo_risk_score", 0.0).to_numpy(dtype=float)

        volume = sku_df.get("volume_unit", 10.0).to_numpy(dtype=float)

        weight = sku_df.get("weight_unit", 5.0).to_numpy(dtype=float)
        ...
        for j, depot in enumerate(depots):
            ...
            holding_cost[:, j] = unit_cost * 0.20 * params["holding_cost_mult"]

            transport_cost[:, j] = (
                q_star * unit_cost * 0.05 * params["transport_cost_mult"]
            )

            risk_cost[:, j] = geo_risk * unit_cost * 0.15

        total_cost = holding_cost + transport_cost + risk_cost
        ...
        prob += pulp.lpSum(
            total_cost[i, j] * y[(i, j)] for i in range(n) for j in range(len(depots))
        )
        ...
        for i in range(n):
            ...
            prob += (
                pulp.lpSum(y[(i, j)] for j in range(len(depots))) == 1,
        ...
        for j, depot in enumerate(depots):
            ...
            prob += (
                pulp.lpSum(volume[i] * y[(i, j)] for i in range(n))
        ...
        for j, depot in enumerate(depots):
            ...
            prob += (
                pulp.lpSum(weight[i] * y[(i, j)] for i in range(n))
        ...
        temp_dir = tempfile.mkdtemp(prefix="cbc_solver_")
    ...
    def get_sensitivity_report(self) -> Dict:
        ...
        for name, constraint in self.prob.constraints.items():
            ...
            if "Capacity" in name:
                ...
                report[name] = {
                    "dual_value": round(constraint.pi, 4),
                    "slack": round(constraint.slack, 4),
                }
    ...
    def solve_with_tracking(
        self, sku_df: pd.DataFrame, experiment_name: str = "W25_Optimization"
    ):

        mlflow.set_experiment(experiment_name)

        with mlflow.start_run(run_name=f"MILP_{int(time.time())}"):

            result = self.solve(sku_df)

            mlflow.log_param("n_skus", len(sku_df))
            ...
            sensitivity = self.get_sensitivity_report()

            for name, values in sensitivity.items():

                mlflow.log_metric(f"dual_{name}", values["dual_value"])
