...
def fig_amplification_heatmap(bw_df: pd.DataFrame) -> go.Figure:

    echelon_cols = [c for c in bw_df.columns if c.startswith("bwr_e")]
...
def export_all_figures(
        ...
        [
            {
                ...
                **{f"bwr_e{i}": b for i, b in enumerate(r.amplification_ratios)},
            }
    ...
    figs = {
        "01_bwr_by_echelon": fig_bwr_by_echelon(summary),
        "02_policy_comparison": fig_policy_comparison(comparison_df),
        "03_bwr_distribution": fig_bwr_distribution_by_tier(bw_df),
        "04_amplification_heatmap": fig_amplification_heatmap(bw_df),
    }

    # single sample SKU
    if summary.sku_results:

        figs["05_sample_timeseries"] = fig_order_demand_timeseries(
