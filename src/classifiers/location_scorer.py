...
class LocationScorer:
    ...
    def score_sku_master(
        ...
        df["base_position_score"] = (
            df["depot_tier"].map(mapping).fillna(self.rear_score)
        )

        raw_score = df["base_position_score"] * df["environment_multiplier"]
        ...
        if max_raw == min_raw:
            ...
            df["location_score_adj"] = (raw_score - min_raw) / (max_raw - min_raw)
    ...
    def score(
        ...
        if max_raw == min_raw:
            ...
            df["location_score"] = (raw_score - min_raw) / (max_raw - min_raw)
