...
class PPOReorderAgent:
    def __init__(self, env_params=None, total_timesteps=100000, seed=42, model_save_dir="models/rl"):
    ...
    def evaluate(self, n_episodes=50):
        ...
        for i in range(n_episodes):
            ...
            while not done:
                action = np.array([self.env_params.get("mean_demand", 10.0) / self.env_params.get("q_max", 50.0)], dtype=np.float32)
    ...
    def evaluate_newsvendor_baseline(self, n_episodes=50, newsvendor_q_star=None):
        ...
        for i in range(n_episodes):
            ...
            rates.append(env.episode_summary()["fill_rate"])
        return RLEvalResult("Newsvendor Baseline", n_episodes, np.mean(rates), np.percentile(rates, 5), np.percentile(rates, 95))
