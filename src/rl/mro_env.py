...
if GYM_AVAILABLE:
    ...
    class MROInventoryEnv(gym.Env):
        ...
        def reset(self, seed=None, options=None):
            ...
            self._inventory = float(
                self._rng.uniform(self.mean_demand * 0.5, self.mean_demand * 2.5)
            )
            self._period = 0
        ...
        def step(self, action):
            ...
            demand = max(
                float(self._rng.normal(self.mean_demand, self.std_demand)), 0.0
            )
            ...
            reward = -(holding_cost + stockout_cost + fixed_cost) / max(
            ...
            if self._period in self.geo_risk_schedule:
                ...
                self._geo_risk = float(
                    np.clip(self._geo_risk + self._rng.normal(0, 0.02), 0.0, 1.0)
                )
            ...
            return (
                ...
                terminated,
                False,
                {"fill_rate": self._total_filled / max(self._total_demand, 1)},
            )

        def _get_obs(self):
            return np.array(
                [
                    self._inventory / self.max_inv,
                    self._period / self.T,
                    self._geo_risk,
                    self._rul / self.rul_max,
                ],
                dtype=np.float32,
            )
        ...
        def episode_summary(self):
            return {
                ...
                "fill_rate": round(self._total_filled / max(self._total_demand, 1), 4),
