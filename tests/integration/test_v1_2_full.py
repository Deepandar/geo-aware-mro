...
def test_pipeline_returns_dataframe(v12_df):
    ...
    assert isinstance(v12_df, pd.DataFrame)
...
def test_required_columns_present(v12_df):
    ...
    missing = [c for c in V1_2_REQUIRED_COLUMNS if c not in v12_df.columns]
...
def test_ci_score_range(v12_df):
    ...
    assert v12_df["ci_score"].between(0, 1).all()


def test_geo_risk_range(v12_df):

    assert v12_df["geo_risk_score"].between(0, 1).all()


def test_tsl_range(v12_df):

    assert v12_df["tsl"].between(0, 1).all()


def test_reputation_score_range(v12_df):

    assert v12_df["reputation_score"].between(0, 1).all()


def test_q_star_non_negative(v12_df):

    assert (v12_df["q_star"] >= 0).all()


def test_rul_signal_positive(v12_df):

    assert (v12_df["rul_signal"] > 0).all()
...
def test_decoupling_modes_valid(v12_df):
    ...
    assert set(v12_df["decoupling_mode"].unique()).issubset(valid)


def test_folk_theorem_satisfied(v12_df):

    assert v12_df["delta_satisfied"].mean() > 0.5


def test_reputation_actions_exist(v12_df):

    assert v12_df["recommended_action"].nunique() >= 3
...
def test_v12_parquet_written():
    ...
    path = Path("data/processed/sku_master_v1.2.parquet")
