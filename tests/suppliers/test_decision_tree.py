...
def qualifier():
    ...
    return DecisionTreeQualifier(max_depth=5)
...
def test_fit_returns_result(
    ...
    result = qualifier.fit(sample_sku_df)

    assert result.n_skus == len(sample_sku_df)
...
def test_predict_adds_columns(
    ...
    qualifier.fit(sample_sku_df)

    out = qualifier.predict(sample_sku_df)

    assert "supplier_risk_class" in out.columns
...
def test_risk_scores_range(
    ...
    assert out["supplier_risk_score"].between(0, 1).all()
