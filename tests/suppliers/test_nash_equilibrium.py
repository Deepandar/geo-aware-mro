...
def test_buffer_non_negative(
    ...
    assert (df_out["buffer_stock_signal"] >= 0).all()
...
def test_high_risk_vital():
    ...
    assert df_out["sourcing_strategy"].iloc[0] == "Dual-Source (Mandatory)"
...
def test_low_risk_single_source():
    ...
    assert df_out["sourcing_strategy"].iloc[0] == "Single-Source"
