...
def injector():
    ...
    return ScenarioInjector("config/scenario_library.yaml")
    ...
    return pd.DataFrame(
        {
            ...
            "mean_lead_time": np.random.uniform(
                14,
                180,
                n,
            ),
...
def test_all_scenarios_injectable(
    ...
    for scenario in [
        ...
        assert impact.scenario_name == scenario
...
def test_impact_summary_dataframe(
    ...
    impacts = injector.inject_all(sku_df)

    df = injector.impact_summary(impacts)
