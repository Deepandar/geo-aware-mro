    ...
    return pd.DataFrame(
        {
            ...
            "mean_demand": np.random.uniform(3, 15, n),
            "std_demand": np.random.uniform(1, 5, n),
            "dp_q_star": np.random.uniform(10, 40, n),
            "q_star": np.random.uniform(8, 30, n),
            "rop": np.random.uniform(3, 12, n),
            "base_stock_level": np.random.uniform(15, 45, n),
            "push_qty": np.random.uniform(0, 8, n),
            "pull_trigger": np.random.choice([True, False], n, p=[0.15, 0.85]),
        }
    )
...
def test_echelon0_bwr_equals_1(
    ...
    assert abs(result.echelons[0].bullwhip_ratio - 1.0) < 1e-6
...
def test_compare_policies(
    ...
    comp = model.compare_policies(sku_df)
...
def test_codp_reduces_bullwhip(
    ...
    std = float(comp[comp.policy == "standard"]["mean_total_amp"].iloc[0])

    codp = float(comp[comp.policy == "codp"]["mean_total_amp"].iloc[0])
...
def test_summary_generation(
    ...
    summary = model.analyze_all(sku_df)
    ...
    assert summary.n_skus == len(sku_df)
